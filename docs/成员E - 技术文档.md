...
本项目是面向企业中后台的 AI 知识库管理系统，目标是让企业内部知识能够被结构化管理、可检索、可问答，并进一步被专家 Agent 和外部 Agent 平台消费。

本人负责的核心模块包括：

- 专家 Agent 的配置与生成：支持配置专家 Agent 的名称、描述、知识范围、回答风格、引用开关、对话沉淀开关和系统提示词。
- Skill 生产标准化改造：基于已有 Skill Agent 会话生产链路，对生成出的 Skill 做标准化重构，将其从“绑定知识库的 RAG 壳”调整为“面向具体任务场景的企业知识能力包”。
- Skill 标准化与导出：参考 Anthropic Agent Skills / Claude Code / Codex 等外部 Agent 平台的通用读取方式，规范 Skill 包结构。
- Skill 消费能力：保留平台内部消费能力，同时打通外部 Agent 通过 Skill 包和平台 Runtime 调用知识库能力的链路。
- Skill CRUD 管理：新增 Skill 管理页和详情页，支持查看、筛选、发布、下载、安装说明、测试、校验、启用、停用和删除。

业务上，本模块服务于集团信息系统部覆盖的人事、财务、法务、采购、审批、职场、安全、隐私、合规、AIGC 等企业场景。系统中的知识库是资源依赖，专家 Agent 和 Skill 是面向具体业务任务的消费入口。

## 2. 总体技术方案

### 2.1 专家 Agent 与 Skill 的关系

专家 Agent 和 Skill 都是知识消费入口，但定位不同：

| 模块 | 定位 | 使用对象 | 主要能力 |
| --- | --- | --- | --- |
| 专家 Agent | 平台内部专家问答角色 | 平台用户 | 绑定知识范围、生成系统提示词、参与聊天问答 |
| Skill | 面向任务场景的能力包 | 外部 Agent / 平台用户 | 标准化说明、安装包、运行脚本、平台调用契约 |

专家 Agent 更像“内部专家角色配置”，强调平台内的问答体验。Skill 更像“可分发的任务能力”，强调能被 Codex、Claude Code 等外部 Agent 安装、识别和调用。

### 2.2 Skill Package 与 Skill Runtime
...
二者在代码逻辑中拆分，但在导出结果中合并。最终用户拿到的仍然是一个 Skill 文件夹或 zip 包，里面既有说明文件，也有平台调用契约。

这样做的原因是：

- 外部 Agent 更适合读取本地文件说明，例如 `SKILL.md`、examples、references。
- 企业知识库检索、权限、引用、日志等能力仍需要由本平台执行。
- 通过 Package + Runtime 的组合，既能兼容外部 Agent 的 Skill 标准，又不丢失平台知识库的治理能力。
...
其中 `knowledgeScope` 使用 JSON 字符串保存，结构在前端和服务层中定义为：
...
这样设计的原因是知识库、分类、标签、知识条目等基础能力在项目中并行开发，先使用通用 JSON 范围配置可以减少跨模块强耦合，后续再替换为真实选择器和更细粒度权限控制。

### 3.3 Agent 生成逻辑

专家 Agent 的生成重点是 system prompt。平台会根据 Agent 的名称、职责描述、回答风格、知识范围和引用要求生成系统提示词，使其能够在问答时形成稳定角色。

核心思路：

```text
用户配置 Agent
→ 平台生成或保存 system prompt
→ 对话时读取 Agent 配置
→ 根据 knowledgeScope 调用 RAG 检索
→ 组装上下文和引用规则
→ 调用 LLM 生成回答
```
...
早期 Skill 更像“绑定某个知识库的 RAG 壳”，不同 Skill 的差异主要是知识库不同。这会带来一个问题：如果所有 Skill 都只是“查知识库”，外部 Agent 很难判断什么时候应该使用某个 Skill，也不利于任务级分发。

因此本人在已有 Skill 生产基础上，重点将 Skill 的身份从“知识库绑定”调整为“任务场景”：

```text
知识库是资源依赖，任务场景才是 Skill 的身份。
```

例如采购流程 Skill 的身份不是“查询采购知识库”，而是：
...
Skill 标准化主要参考 Anthropic Agent Skills 的设计思路，尤其是：

- 以 `SKILL.md` 作为外部 Agent 的主入口。
- `SKILL.md` 不写过多冗余内容，只保留关键触发条件、使用方式、边界和输出规范。
- 更详细的信息拆到 references 文件中，由 Agent 需要时再读取。
- 用脚本辅助外部 Agent 调用平台能力。

在本项目中，最终目标是让 Skill 能被 Claude Code、Codex 等常用 Agent 平台识别和安装。
...
为了避免用户每次从零补齐标准化信息，本次改造在 Skill 生产结果上加入了默认模板：
...
这套默认配置降低了 Skill 生产门槛，同时保证外部 Agent 有足够信息判断是否调用该 Skill。
...
`SkillVersion` 用于保存发布时的 manifest 和快照，便于追踪版本。

`SkillApiKey` 保存外部调用密钥摘要，明文密钥只在发布后展示一次。

`SkillRunLog` 记录每次 Skill Runtime 调用的输入、输出、引用数量和错误信息，方便审计和排查。
...
其中 `SkillTaskScenario` 是本次标准化的重点：
...
这使 Skill 不再只依赖知识库，而是拥有明确任务身份。

## 6. Skill 消费方式与原理

### 6.1 平台内部消费

平台内部仍然保留 Skill 消费能力。对话执行时可以扫描已发布 Skill，根据 Skill 描述和任务场景判断是否适合调用。
...
本项目没有把知识库全文、权限逻辑和 LLM 调用全部放入 Skill 包，而是保留平台 Runtime，原因是：

- 企业知识库可能持续更新，本地包不适合保存完整知识。
- 需要保留引用来源、调用日志和后续审计能力。
- 运行密钥可以被平台控制和停用。
- RAG 检索、LLM 模型和 citation 逻辑可以统一升级，不需要每个外部 Agent 重新安装。
...
为了保证 Skill 不是泛化的知识库查询工具，发布前加入了质量校验。
...
这样前端可以用 UI 友好的方式展示发布风险，而不是只返回简单的 schema 错误。
...
同时在已有 Skill Agent 会话页提供跳转到 Skill 管理的入口，新建 Skill 仍然回到 Skill Agent 会话中完成。

这样设计是因为当前项目已有的 Skill 生产入口是对话式 Skill Agent。本人没有重做 Skill Agent 本身，而是在该生产链路之后补齐标准化、管理、测试、发布、安装和删除能力。
...
### 10.4 为什么新建 Skill 仍然走已有 Skill Agent
...
本人补充的 Skill 标准化与管理页负责治理
```

这更符合实际用户流程：先通过对话生成初稿，再在管理页测试、校验、发布和安装。
...
→ 本次改造将 Skill 草稿标准化为可导出的 Skill 包
...
- 通过 CRUD 管理页提升可运维性，方便发布、停用、删除和测试。
...
成员 E 负责的工作从专家 Agent 配置开始，逐步扩展到 Skill 标准化生产、外部消费和后台管理。最终形成的方案不是简单的“知识库问答壳”，而是将企业知识库包装成可被内部专家 Agent 和外部 Agent 平台消费的任务能力。

该方案的核心判断是：企业知识库是底层资源，真正能被用户和 Agent 理解的是任务场景。因此 Skill 的标准化重点不只是导出文件，而是明确任务身份、触发条件、边界、输出格式和平台调用契约。通过这种方式，项目能够把集团信息系统部覆盖的复杂业务知识转化为可安装、可调用、可治理的企业 AI 能力。
