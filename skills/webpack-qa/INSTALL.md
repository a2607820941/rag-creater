...
- Codex: `$CODEX_HOME/skills/webpack-qa`, or `~/.codex/skills/webpack-qa` when `CODEX_HOME` is not set.
- Claude Code: `$CLAUDE_HOME/skills/webpack-qa`, or `~/.claude/skills/webpack-qa` when `CLAUDE_HOME` is not set.

To install somewhere else:

```bash
node scripts/install-skill.mjs codex --target /path/to/skills/webpack-qa
node scripts/install-skill.mjs claude-code --target /path/to/skills/webpack-qa
```
...
```bash
cd ~/.codex/skills/webpack-qa
node scripts/set-runtime-key.mjs "<api-key>"

cd ~/.claude/skills/webpack-qa
node scripts/set-runtime-key.mjs "<api-key>"
```
