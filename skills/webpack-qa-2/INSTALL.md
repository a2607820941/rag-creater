...
- Codex: `$CODEX_HOME/skills/webpack-qa-2`, or `~/.codex/skills/webpack-qa-2` when `CODEX_HOME` is not set.
- Claude Code: `$CLAUDE_HOME/skills/webpack-qa-2`, or `~/.claude/skills/webpack-qa-2` when `CLAUDE_HOME` is not set.

To install somewhere else:

```bash
node scripts/install-skill.mjs codex --target /path/to/skills/webpack-qa-2
node scripts/install-skill.mjs claude-code --target /path/to/skills/webpack-qa-2
```
...
```bash
cd ~/.codex/skills/webpack-qa-2
node scripts/set-runtime-key.mjs "<api-key>"

cd ~/.claude/skills/webpack-qa-2
node scripts/set-runtime-key.mjs "<api-key>"
```
