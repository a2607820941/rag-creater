...
- Codex: `$CODEX_HOME/skills/after-sales-qa`, or `~/.codex/skills/after-sales-qa` when `CODEX_HOME` is not set.
- Claude Code: `$CLAUDE_HOME/skills/after-sales-qa`, or `~/.claude/skills/after-sales-qa` when `CLAUDE_HOME` is not set.

To install somewhere else:

```bash
node scripts/install-skill.mjs codex --target /path/to/skills/after-sales-qa
node scripts/install-skill.mjs claude-code --target /path/to/skills/after-sales-qa
```
...
```bash
cd ~/.codex/skills/after-sales-qa
node scripts/set-runtime-key.mjs "<api-key>"

cd ~/.claude/skills/after-sales-qa
node scripts/set-runtime-key.mjs "<api-key>"
```
